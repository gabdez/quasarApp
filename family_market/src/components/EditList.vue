    ...
    <div>
        ...
        <div class="q-mt-md">
            ...
            <div class="container q-mt-sm">
                ...
                <q-card flat class="cardUser cardAddUser" @click="addModal = true">
        ...
        <q-dialog v-model="addModal">
            <q-card style="min-width: 300px;">
                <q-card-section class="row items-center">
                    ...
                    <q-space/>
                    <q-btn icon="close" size="15px" @click="addModal = false" flat round dense/>
                </q-card-section>

                <q-card-section>
                    <q-input rounded outlined dense v-model="searchUsername" placeholder="username">
                        <template v-slot:append>
                            <q-icon v-if="searchUsername" name="close" @click.stop="searchUsername = null"></q-icon>
                        </template>
                ...
                <q-card-section>
                    <q-list bordered separator>
                        ...
                        <q-item v-else v-for="user in usersFind" :key="user.email + '-add'">
                            <q-item-section avatar>
                                <q-avatar>
                                    <img :src="'statics/avatars/' + user.url">
                                </q-avatar>
                            ...
                            <q-item-section side>
                                <q-btn small outline color="red" icon="ion-ios-add" @click="addUser(user)"/>
                            </q-item-section>
                        </q-item>
                    </q-list>
                </q-card-section>
        ...
        <q-dialog v-model="confirm" persistent>
            <q-card class="cardRadius">
                ...
                <q-card-actions class="cardAction">
                    <q-btn flat class="btnL q-ma-none" label="Cancel" color="primary" @click="cancel"/>
                    <q-btn flat label="Delete" class="btnR" color="red-14" @click="deleteUser"/>
                </q-card-actions>
            </q-card>
        </q-dialog>
    </div>
        ...
        addUser(user) {
            if (!this.list.users.includes(user.id)) {
                ...
                this.$store
                    ...
                    .then(() => {
                        this.$q.notify({
                            message: user.username + " added successfully to the list!",
                            color: "green",
                            timeout: 2500
                        });
