            ...
            <span class="listArticle">
                ...
                <q-item v-if="items.length == 0" :key="'noArticle'">
                    <q-item-section class="text-center"
                        >no article in the shopping cart yet</q-item-section
                    >
                </q-item>
            </span>
            <q-btn
                class="fixed"
                style="bottom: 170px; right: 25px;"
                round
                color="red"
                icon="add"
                @click="showModal"
    ...
    methods: {
        addItem() {
            ...
            this.scrollToElement(
                document.getElementsByClassName("listArticle")[0]
            );
        ...
        clearCompleted() {
            var itemsCompleted = this.$store.getters[
                "lists/getAllUncheckedItems"
            ](this.list.id);
            console.log(itemsCompleted);
            var idList = this.list.id;
            this.$store
                .dispatch("lists/clearCompletedItem", {
                    idList,
                    itemsCompleted
                })
                .then(() => {
        ...
        setLocalStorage() {
            LocalStorage.set(
                "list_todo_market",
                this.$store.getters["lists/getAllLists"]
            );
        },
        showModal() {
    ...
    computed: {
        items() {
            return this.hideCompleted
                ? this.$store.getters["lists/getAllUncheckedItems"](
                      this.list.id
                  )
        ...
        styleList() {
            let height =
                this.heightLabel != 0 ? this.heightLabel + "px" : "0px";
